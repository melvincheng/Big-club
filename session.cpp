...
void Session::create() {
  ...
  std::size_t nonAlpha = name.find_first_not_of("abcdefghijklmnopqrstuvwxyz- ");
  ...
  balance = float(int(balance*100+0.5))/100;
  std::cout << "A new account was made under the name:\n" << name << '\n' <<  "with a current balance of:\n" << balance << std::endl << "Your account will be available within one buisness day" << std::endl;
  return;
}
